...
function publicState(room) {
  return {
    ...
    players: room.players.map(p => ({
      ...
      // In hiddenRole mode, don't reveal roles during play
...
function tallyVotes(room) {
  ...
  const isImpostor = undercoverIds.includes(eliminatedId);
  const unanimousVote = Object.values(room.votes).every(v => v === eliminatedId);

  // Civilians who voted for impostor get points
  Object.entries(room.votes).forEach(([voterId, targetId]) => {
    const voter = room.players.find(p => p.id === voterId);
    if (!voter) return;
    if (undercoverIds.includes(targetId)) {
      voter.score += 2;
      if (unanimousVote) voter.score += 1; // bonus unanimity
    }
  });

  // Impostors not voted out get points
